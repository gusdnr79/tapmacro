...
3. 설치 → 권한 부여 → 필요 시 캡처 동의 자동 처리.
   - 저장소가 공개이므로 실기기 테스트 시작 전에 방해 금지 모드를 켠다 (`adb shell cmd notification set_dnd priority`, 종료 시 원래 값으로 복구). 실패하면 사용자에게 방해 금지 모드를 켜달라고 요청한다.
   - 스크린샷은 저장 전에 알림이나 다른 앱 화면이 없는지 확인하고, 있으면 버리고 다시 찍는다.
...
- 증거 파일 없는 PASS
- 기기 시리얼, 계정, 전화번호 등 개인 식별 정보를 증거나 보고서에 기록 (로그에 섞이면 가림 처리)
