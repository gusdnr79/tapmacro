...
- 저장소: GitHub **공개(Public)**. 누구나 볼 수 있다. 로컬 경로는 `C:\dev\` 아래 (OneDrive 동기화 폴더 금지)
...
8. 저장소는 공개다. 비밀값, 개인 정보(실명, 전화번호, 이메일, 기기 시리얼 전체), 회사 관련 정보를 코드, 로그, 보고서, 커밋 메시지, Issue, PR에 넣지 않는다.
9. 스크린샷 증거는 테스트 앱 화면만 담는다. 알림, 상태바 개인 정보, 다른 앱 화면이 찍히면 저장하지 않는다. 사용자가 준 이미지는 개인 정보가 없는지 확인하고, 있으면 저장하지 말고 알린다.
